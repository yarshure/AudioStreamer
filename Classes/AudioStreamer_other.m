...
- (void)enqueueBuffer
{
	@synchronized(self)
	{
        ...
        if (fileTypeHint == kAudioFilePUREType) {
            NSLog(@"fillBufferIndex enque %i",fillBufferIndex);
            short *coreAudiobuffer;
            coreAudiobuffer = (short*) fillBuf->mAudioData;
            BYTE  frame_buffer2[644];
            
            memset(frame_buffer2,0,sizeof(frame_buffer2));
            memcpy(frame_buffer2,coreAudiobuffer,sizeof(frame_buffer2));
            //memset(coreAudiobuffer,0,sizeof(coreAudiobuffer));
            Pure_DecodeFrame(pCODEC, frame_buffer2, coreAudiobuffer);
        ...
        {//fixme
            ...
            @synchronized(self)
            {
                ...
                if (fileTypeHint != kAudioFilePUREType){
                    ...
                    if (bufSpaceRemaining <=inNumberBytes) {
                        //copySize = bufSpaceRemaining;//满
                        memcpy((coreAudiobuffer+bytesFilled), (const char*)(inInputData+offset), copySize);
                        packetsFilled += 1;
                        bytesFilled = 0;
                        NSLog(@"0000 fillBufferIndex %i,copySize= %zu, bytesFilled= %zu",fillBufferIndex,copySize,bytesFilled);
                    }else {
                        //copySize = inNumberBytes;//不满
                        memcpy((coreAudiobuffer+bytesFilled), (const char*)(inInputData+offset), copySize);
                        bytesFilled += copySize;
                        NSLog(@"1111 fillBufferIndex %i,copySize= %zu, bytesFilled= %zu",fillBufferIndex,copySize,bytesFilled);
                        break;
                    }
                    ...
                    if (bytesFilled >0 ) {//&& bufSpaceRemaining<=inNumberBytes
                        //
                        memcpy((coreAudiobuffer+bytesFilled), (const char*)(inInputData), copySize);
                        
                        if ((copySize+bytesFilled) == frame_bytes) {
                            //if (memcpy(coreAudiobuffer,frame_buffer,644)){//Pure_DecodeFrame(pCODEC, frame_buffer, coreAudiobuffer
                                //decode
                        ...
                        if (inNumberBytes < frame_bytes) {// && inNumberBytes != 0
                            //memset(frame_buffer, 0, sizeof(frame_buffer));
                            memcpy(coreAudiobuffer, (const char*)(inInputData+offset), inNumberBytes);
                            bytesFilled = inNumberBytes;
                            copySize = inNumberBytes;
                            NSLog(@"fillBufferIndex need ++22");
                            return;
                        }else{
                            
                            if (offset >=inBytes) {
                                //reConnect = YES;
                                DLog(@"error found,丢包，跳数据 fixme, now use seek reconnect");
                                //[self pause];
                                //self.state = AS_INITIALIZED;
                                [self seekToTime:lastProgress];
                                return;
                            ...
                            if (memcpy(coreAudiobuffer,(BYTE*)(inInputData+offset),frame_bytes) )
                            {
                                ...
                                DLog(@"decode error -%lu,%ld,%s",copySize,(long)frame_bytes,sel_getName(_cmd));
                                [self failWithErrorCode:AS_FILE_STREAM_GET_PROPERTY_FAILED];
                                //return;
                            } 
                        }
